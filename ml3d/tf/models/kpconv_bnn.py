...
class KPFBNN(BaseModel):
    """Class defining KPFBNN.
    ...
    def __init__(
        ...
        self.C = len(lbl_values) - len(ign_lbls) # classes to segment
    ...
    def transform(self,
        ...
        batch_inds = self.get_batch_inds(stacks_lengths) # batch meaning ball
    ...
    def inference_end(self, results):
        ...
        for len in self.test_meta['lens']:
            r += len
            self.test_probs[inds[l:r]] = self.test_probs[inds[l:r]] * test_smooth + (1 - test_smooth) * results[l:r]
            l += len
    ...
    def transform_inference(self, data):
        ...
        n_points = 0 # number of overall sampled points
        points = np.array(data['search_tree'].data)
        # 
        while (n_points < cfg.batch_limit): # sample 10_000 points
            cloud_ind = 0
            # get random point index from point cloud with lowest possibility in posibility table
            point_ind = int(np.argmin(self.possibility)) # possibility = random[num_points] between 0 and 1000
            # get point with that index
            center_point = points[point_ind, :].reshape(1, -1)
            pick_point = center_point.copy()
            # get point indices within in_radius(e.g. r=1.5 for S3DIS, r=4.0 for PL3D and r=6.0 for Semantic3D)
            input_inds = data['search_tree'].query_radius(pick_point,
                                                          r=cfg.in_radius)[0]

            # number of points inside ball
            ...
            dists = np.sum(np.square(
                (points[input_inds] - pick_point).astype(np.float32)),
                           axis=1)
            # reduce probability of the close points being picked up next by adding their proximity to the center the the possibility table                            
            ...
            # center the picked points on the origin 
            input_points = points[input_inds].copy() - pick_point
            feat = data['feat']
            
            # normalize features (colors if there are any and/or points if cfg)
            t_normalize = self.cfg.get('t_normalize', {})
            input_points, feat = trans_normalize(input_points, feat,
                                                 t_normalize)

            # get coords to use as features and append existing input features
            if feat is None:
                coords = input_points
            else:
            ...
            # add back z-component to the z- feature
            coords[:, 2] += pick_point[:, 2]

            if len(data['label'][input_inds].shape) == 2: # unclear -> why would there be two labels per point
                input_labels = data['label'][input_inds][:, 0]
            else:
                input_labels = data['label'][input_inds]

            if n > 0:
                p_list += [input_points] # centered points
                c_list += [coords] # coords = coords + features (should be 6?)
                pl_list += [input_labels] # labels
                pi_list += [input_inds] # indices of subcloud points in scene
                ci_list += [cloud_ind] # 0

        # concatenate all points, colors and labels in batch
        stacked_points = np.concatenate(p_list, axis=0), 
        stacked_colors = np.concatenate(c_list, axis=0), 
        point_labels = np.concatenate(pl_list, axis=0), 
        
        # get number of points in each sub cloud
        stacks_lengths = np.array([tp.shape[0] for tp in p_list],
                                  dtype=np.int32),
        # concatenate indices of sub point cloud points to reconstruct scene                                  
        point_inds = np.concatenate(pi_list, axis=0),
        cloud_inds = np.array(ci_list, dtype=np.int32) # 000?

        input_list = self.transform( # TODO: check what call to transform() does
...
MODEL._register_module(KPFBNN, 'tf')
