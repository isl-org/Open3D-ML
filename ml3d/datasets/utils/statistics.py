...
def compute_scene_stats(mesh_vertices, semantic_labels, instance_labels,
    ...
    Args:
        ...
        instance_objects (List[BoundingBox3D]):
    ...
    if instance_objects is not None:
        class_id, class_bbox_count = np.unique(
            [obj.label_class for obj in instance_objects], return_counts=True)
        ...
        for obj in instance_objects:
            stats['bbox_shape'][obj.label_class].append(obj.size)
        if (hasattr(instance_objects[0], 'truncation') and
                instance_objects[0].truncation is not None):
            stats['truncation'], _ = np.histogram(
                [obj.truncation for obj in instance_objects],
                bins=NBINS,
                range=(0, 1))
        if (hasattr(instance_objects[0], 'n_pts_inside') and
                instance_objects[0].n_pts_inside is not None):
            stats['n_pts_inside'] = [
                obj.n_pts_inside for obj in instance_objects
            ]
...
def compute_dataset_stats(scene_stats):
    ...
    if 'n_pts_inside' in scene_stats[0]:
        n_pts_bbox = np.concatenate(
            [stats['n_pts_inside'] for stats in scene_stats])
        hist = np.histogram(n_pts_bbox, bins=NBINS)
        data_stats['n_pts_inside'] = {
    ...
    if 'n_pts_seg' in scene_stats[0]:
        ...
        for class_id in class_ids:
            class_n_pts_segs = []
            for stats in scene_stats:
                if class_id in stats['n_pts_seg']:
                    class_n_pts_segs.extend(stats['n_pts_seg'][class_id])
            ...
            } if len(class_n_pts_segs) > 0 else {}
...
def calc_anchor_boxes(bbox_shapes, n_anchors):
    ...
    Euclidean distance on the bounding box size.
