...
def avg_voxelize(feat, coords, r):
    ...
    # grid_x = torch.zeros(r * r * r,).scatter_add_(0, hash, feat[:, 0]).reshape(
    #     (r, r, r, 1))
    # grid_y = torch.zeros(r * r * r,).scatter_add_(0, hash, feat[:, 1]).reshape(
    #     (r, r, r, 1))
    # grid_z = torch.zeros(r * r * r,).scatter_add_(0, hash, feat[:, 2]).reshape(
    #     (r, r, r, 1))
    # count = torch.zeros(r * r * r,).scatter_add_(
    #     0, hash, torch.ones_like(feat[:, 0])).reshape((r, r, r, 1)).clamp(min=1)
    # grid = torch.cat([grid_x, grid_y, grid_z], 3) / count
