...
class SparseConvUnet(BaseModel):
    def __init__(self,
        ...
        cfg = self.cfg
        self.m = cfg.m
        self.ssc = SubmanifoldSparseConv(in_channels=in_channels, filters=m, kernel_size=[3, 3, 3])
        self.unet = UNet(1, [m, 2*m, 3*m, 4*m, 5*m, 6*m, 7*m], False)
    ...
    def preprocess(self, data, attr):
        ...
        offset = np.clip(4096 - points.max(0) + points.min(0) - 0.001, 0, None)/2
        ...
        points = (points.astype(np.int32) + 0.5).astype(np.float32)  # Move points to voxel center.

        # Take one point from each voxel. Ideally we want to take average of features from same voxel.
        _, index, inv = np.unique(points, return_index=True, return_inverse=True, axis=0)
        index = np.sort(index)
    ...
    def inference_end(self, inputs, results):
        ...
        probs = np.reshape(results, [-1, self.cfg.num_classes])[inputs['proj_inds']]

        pred_l = np.argmax(probs, 1)
        print(pred_l[:30])

        return {'inference_labels': pred_l,
...
class SubmanifoldSparseConv(nn.Module):
    def __init__(self, in_channels, filters, kernel_size, use_bias=False, offset=None, normalize=False):
        ...
        self.net = ml3d.layers.SparseConv(in_channels=in_channels, filters=filters, kernel_size=kernel_size, use_bias=use_bias, offset=offset, normalize=normalize)
...
class DeConvolution(nn.Module):
    def __init__(self, in_channels, filters, kernel_size, use_bias=False, offset=None, normalize=False):
        ...
        offset = torch.full((3,), offset, dtype=torch.float32)
        self.net = ml3d.layers.SparseConvTranspose(in_channels=in_channels, filters=filters, kernel_size=kernel_size, use_bias=use_bias, offset=offset, normalize=normalize)
...
class UNet(nn.Module):
    def __init__(self, reps, nPlanes, resudual_blocks=False, downsample=[2, 2], leakiness=0):
    ...
    def block(m, a, b):
        ...
        m.append(SubmanifoldSparseConv(in_channels=a, filters=b, kernel_size=[3, 3, 3]))
    ...
    def U(nPlanes):
        ...
        if len(nPlanes) > 1:
            ...
            m.append(nn.LeakyReLU(0))
            m.append(Convolution(in_channels=nPlanes[0], filters=nPlanes[1], kernel_size=[2, 2, 2]))
            m = m + UNet.U(nPlanes[1:])
            m.append(nn.BatchNorm1d(nPlanes[1], eps=1e-4, momentum=0.01))
            m.append(nn.LeakyReLU(0))
            m.append(DeConvolution(in_channels=nPlanes[1], filters=nPlanes[0], kernel_size=[2, 2, 2]))
...
def load_unet_wts(net, path):
    ...
    for key in state_dict.keys():
        ...
        if len(wts[i].shape) == 4:
            shp = wts[i].shape
            state_dict[key] = np.transpose(wts[i].reshape(int(shp[0]**(1/3)), int(shp[0]**(1/3)), int(shp[0]**(1/3)), shp[-2], shp[-1]), (2, 1, 0, 3, 4))
        else:
