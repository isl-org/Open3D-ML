...
from ..utils.objdet_helper import Anchor3DRangeGenerator, DeltaXYZWLHRBBoxCoder, xywhr2xyxyr, LiDARInstance3DBoxes, box3d_multiclass_nms, limit_period
...
class Anchor3DHead(nn.Module):
    ...
    def forward(self, feats):
        ...
        """
        results = map(self.forward_single, feats)
    ...
    def get_loss(self, results, inputs, device):
        loss_cls=dict(
            ...
            alpha=0.25,
            loss_weight=1.0),
        loss_cls=FocalLoss(2.0, 0.25)
        loss_bbox=nn.SmoothL1Loss #(dict(type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=2.0),
        loss_dir=nn.CrossEntropyLoss #loss_weight=0.2)))
