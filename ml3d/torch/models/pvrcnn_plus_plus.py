...
class PVRCNNPlusPlus(BaseModel):
    ...
    def __init__(self,
        ...
        self.to(device)
        self.voxel_layer = PVRCNNPlusPlusVoxelization(
            point_cloud_range=point_cloud_range, **voxelize)
        self.sparseconvbackbone = PVRCNNPlusPlusBackbone3D(
            point_cloud_range=point_cloud_range, **voxel_encoder)
        self.bev_module = PVRCNNPlusPlusBEVModule(
            point_cloud_range=point_cloud_range)
        self.backbone_2d = PVRCNNPlusPlusBackbone2D(640, **backbone_2d)
        self.rpn_module = RPNModule(input_channels=512,
                                    device=device,
                                    point_cloud_range=point_cloud_range,
                                    **rpn_module)
    ...
    def voxelize(self, points):
        ...
        for res in points:
            ...
            normalizer = torch.clamp_min(res_num_points.view(-1, 1),
                                         min=1.0).type_as(res_voxels)
    ...
    def convert_open3d_voxel_to_dense(self,
                                      x_open3d,
                                      x_pos_open3d,
                                      voxel_size=1.0):
        x_sparse = []
        ...
        for x, x_pos in zip(x_open3d, x_pos_open3d):
            dense_shape = (
                ...
                (np.array(voxel_size) * 8) + 1).astype(int)[::-1].tolist()
            dense_shape.append(x.shape[1])
            x_sparse_temp = torch.sparse_coo_tensor(
                torch.tensor(x_pos).t(), x, dense_shape)
            x_sparse.append(x_sparse_temp)
        return x_sparse

    def backbone_3d(self, points):
        voxels, voxel_features, num_points, coors, coors_batch = self.voxelize(
            points)
        x_intermediate_layers, x, x_pos = self.sparseconvbackbone(
    ...
    def forward(self, inputs):
        ...
        targets_dict = self.box_refinement.assign_targets(len(gt_boxes),
                                                          rois,
                                                          scores=roi_scores,
                                                          _gt_boxes=gt_boxes,
        ...
        outs = self.box_refinement(point_coords, point_features, rois,
                                   roi_scores, roi_labels, keypoint_weights,
                                   roi_targets_dict, gt_boxes, gt_labels)
    ...
    def get_loss(self, result, data):
        ...
        if not self.keypoints_not_found:
            loss_point, tb_dict = self.keypoint_weight_computer.get_loss(
                tb_dict)
    ...
    def transform(self, data, attr):
        ...
        if attr['split'] not in ['test', 'testing']:
            ...
            cond = (t_data["labels"] >= 0) & (t_data["labels"] < len(
                self.classes))
    ...
    def inference_end(self, results, inputs):
        ...
        for index in range(batch_size):
            ...
            idxs = self.rpn_module.multiclass_nms(
                boxes=box_preds,
                scores=cls_preds,
                class_ids=label_preds,
                score_thr=self.rpn_module.score_thr,
                overlap_thr=0.01)
...
class SequentialSparseConv(nn.Module):
    ...
    def __init__(self,
                 ...
                 downsample=False):
        super().__init__()
        self.conv1 = SparseConvBlock(in_channels,
                                     out_channels,
                                     kernel_size,
                                     point_cloud_range,
                                     voxel_size,
        ...
        self.conv2 = SubmanifoldSparseConvBlock(out_channels,
                                                out_channels,
                                                kernel_size,
                                                norm_fn=norm_fn)
        self.conv3 = SubmanifoldSparseConvBlock(out_channels,
                                                out_channels,
                                                kernel_size,
                                                norm_fn=norm_fn)
...
class PVRCNNPlusPlusBackbone3D(nn.Module):
    ...
    def __init__(self, in_channels, point_cloud_range, voxel_size):
        ...
        self.conv_input = SubmanifoldSparseConvBlock(in_channels,
                                                     16, [3, 3, 3],
                                                     norm_fn=norm_fn)

        self.conv1 = SparseConvBlock(16,
                                     16, [3, 3, 3],
                                     point_cloud_range,
                                     voxel_size,
                                     padding=1,
        ...
        self.conv2 = SequentialSparseConv(16,
                                          32, [3, 3, 3],
                                          point_cloud_range,
                                          voxel_size,
                                          padding=1,
        ...
        self.conv3 = SequentialSparseConv(32,
                                          64, [3, 3, 3],
                                          point_cloud_range,
                                          voxel_size,
                                          padding=1,
        ...
        self.conv4 = SequentialSparseConv(64,
                                          64, [3, 3, 3],
                                          point_cloud_range,
                                          voxel_size,
                                          padding=1,
        ...
        self.conv_out = SparseConvBlock(64,
                                        128, [3, 3, 3],
                                        point_cloud_range,
                                        voxel_size,
                                        padding=0,
                                        norm_fn=norm_fn)  #stride = (2, 1, 1)
...
class PVRCNNPlusPlusBEVModule(nn.Module):
    ...
    def forward(self, sparse_tensor_list):
        ...
        for x_sparse in sparse_tensor_list:
            ...
            spatial_features = spatial_features.contiguous().view(
                1, C * D, H, W)
...
class PVRCNNPlusPlusBackbone2D(nn.Module):
    ...
    def __init__(self, input_channels, layer_nums, layer_strides, num_filters,
...
class RPNHead(nn.Module):
    ...
    def __init__(self, shared_conv_channels, output_channels, num_head_conv):
        ...
        for k in range(num_head_conv - 1):
            conv_list.append(
                nn.Sequential(
                    nn.Conv2d(shared_conv_channels,
                              shared_conv_channels,
                              kernel_size=3,
                              stride=1,
                              padding=1,
                              bias=True), nn.BatchNorm2d(shared_conv_channels),
                    nn.ReLU()))
        conv_list.append(
            nn.Conv2d(shared_conv_channels,
                      output_channels,
...
class RPNModule(nn.Module):
    ...
    def __init__(self, input_channels, device, shared_conv_channels,
                 num_head_conv, class_names, point_cloud_range, voxel_size,
                 feature_map_stride, nms_pre, score_thr,
                 post_center_limit_range, num_max_objs, gaussian_overlap,
                 min_radius, code_weights, cls_weight, loc_weight):
        super().__init__()
        self.point_cloud_range = point_cloud_range
        self.voxel_size = voxel_size
        ...
        self.post_center_limit_range = torch.tensor(post_center_limit_range).to(
            device)

        self.class_id_mapping = torch.from_numpy(
            np.array([self.class_names.index(x) for x in class_names])).cuda()
    ...
    def forward(self, bev_feats, _gt_boxes=None, _gt_labels=None):
        ...
        if True:  #self.training:
            ...
            for i in range(batch_size):
                gt_boxes.append(
                    torch.cat([_gt_boxes[i], _gt_labels[i].view(-1, 1)], dim=1))
        ...
        bboxes, scores, labels = self.generate_predicted_boxes(
            batch_size, preds)
        rois, roi_scores, roi_labels = self.reorder_rois_for_refining(
            batch_size, bboxes, scores, labels)
    ...
    def get_loss(self):
        ...
        pred_boxes = torch.cat(
            [preds[head_num] for head_num in range(len(preds) - 1)], dim=1)
        reg_loss = self.reg_loss_func(pred_boxes, targets_dict['masks'][0],
                                      targets_dict['inds'][0], target_boxes)
    ...
    def assign_target_of_single_head(self,
        ...
        for k in range(min(num_max_objs, gt_boxes.shape[0])):
            ...
            self.draw_gaussian_to_heatmap(heatmap[cur_class_id], center[k],
                                          radius[k].item())
    ...
    def assign_targets(self, gt_boxes, feature_map_size):
        ...
        for bs_idx in range(batch_size):
            ...
            gt_class_names = np.array(
                self.class_names)[cur_gt_boxes[:, -1].cpu().long().numpy()]
    ...
    def generate_predicted_boxes(self, batch_size, preds):
        ...
        for k, final_dict in enumerate(final_pred_dicts):
            ...
            idxs = self.multiclass_nms(bbox_pred,
                                       score_pred,
                                       label_pred,
                                       score_thr=None,
                                       overlap_thr=0.7)
    ...
    def draw_gaussian_to_heatmap(self,
                                 heatmap,
                                 center,
                                 radius,
                                 k=1,
        ...
        masked_gaussian = torch.from_numpy(
            gaussian[radius - top:radius + bottom,
                     radius - left:radius + right]).to(heatmap.device).float()

        if min(masked_gaussian.shape) > 0 and min(
                masked_heatmap.shape) > 0:  # TODO debug
            if valid_mask is not None:
                cur_valid_mask = valid_mask[y - top:y + bottom,
                                            x - left:x + right]
    ...
    def decode_bbox_from_heatmap(self,
        ...
        center = self._transpose_and_gather_feat(center,
                                                 inds).view(batch_size, K, 2)
        rot_sin = self._transpose_and_gather_feat(rot_sin,
                                                  inds).view(batch_size, K, 1)
        rot_cos = self._transpose_and_gather_feat(rot_cos,
                                                  inds).view(batch_size, K, 1)
        center_z = self._transpose_and_gather_feat(center_z,
                                                   inds).view(batch_size, K, 1)
        ...
        if vel is not None:
            vel = self._transpose_and_gather_feat(vel,
                                                  inds).view(batch_size, K, 2)
...
def build_local_aggregation_module(input_channels, config):
    cur_layer = VectorPoolAggregationModuleMSG(input_channels=input_channels,
                                               config=config)
    ...
    def __init__(self, input_channels, config):
        ...
        for k in range(self.num_groups):
            ...
            cur_vector_pool_module = VectorPoolAggregationModule(
                ...
                num_reduced_channels=self.model_cfg.get('NUM_REDUCED_CHANNELS',
                                                        None),
                num_channels_of_local_aggregation=self.model_cfg.
...
class VectorPoolAggregationModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.separate_local_aggregation_layer = nn.Sequential(
            nn.Conv1d(num_c_in,
                      ...
                      bias=False), nn.BatchNorm1d(num_c_out), nn.ReLU())
    ...
    def get_dense_voxels_by_center(point_centers, max_neighbour_distance,
        ...
        x_grids = torch.arange(-R + R / num_voxels[0],
                               R - R / num_voxels[0] + 1e-5,
                               2 * R / num_voxels[0],
                               device=device)
        y_grids = torch.arange(-R + R / num_voxels[1],
                               R - R / num_voxels[1] + 1e-5,
                               2 * R / num_voxels[1],
                               device=device)
        z_grids = torch.arange(-R + R / num_voxels[2],
    ...
    def vector_pool_with_local_interpolate(self, xyz, xyz_batch_cnt, features,
        ...
        voxel_centers = self.get_dense_voxels_by_center(
            ...
            max_neighbour_distance=self.max_neighbour_distance,
            num_voxels=self.num_local_voxel)  # (M1 + M2 + ..., total_voxels, 3)
...
class VectorPoolLocalInterpolateModule(nn.Module):
    ...
    def forward(self, support_xyz, support_features, xyz_batch_cnt, new_xyz,
        ...
        for i in range(xyz_batch_cnt.shape[0]):
            if i == 0:
                current_support_xyz = support_xyz[0:xyz_batch_cnt[i]]
                current_support_features = support_features[0:xyz_batch_cnt[i]]
                current_new_xyz_grid_centers = new_xyz_grid_centers[
                    0:new_xyz_batch_cnt[i]]
            else:
                current_support_xyz = support_xyz[current_sum:current_sum +
                                                  xyz_batch_cnt[i]]
                current_support_features = support_features[
                    current_sum:current_sum + xyz_batch_cnt[i]]
                current_new_xyz_grid_centers = new_xyz_grid_centers[
                    current_sum_new:current_sum_new + new_xyz_batch_cnt[i]]
            current_sum = current_sum + xyz_batch_cnt[i]
            current_sum_new = current_sum_new + new_xyz_batch_cnt[i]
            with torch.no_grad():
                dist_o3d_temp, idx_o3d_temp = o3d_tnn(
                    current_new_xyz_grid_centers.view(1, -1, 3),
                    current_support_xyz.view(1, -1, 3))
                dist_o3d.append(
                    dist_o3d_temp.view(current_new_xyz_grid_centers.shape))
                idx_o3d.append(
                    idx_o3d_temp.view(current_new_xyz_grid_centers.shape))

            dist_recip = 1.0 / (dist_o3d_temp + 1e-8)
            norm = torch.sum(dist_recip, dim=-1, keepdim=True)
            weight = dist_recip / torch.clamp_min(norm, min=1e-8)

            interpolated_feats_o3d = o3d_int(
                current_support_features.view(
                    1, -1,
                    current_support_xyz.view(1, -1, 3).shape[1]),
                idx_o3d_temp.view(1, -1, 3), weight.view(1, -1, 3))
            interpolated_feats_o3d = interpolated_feats_o3d.view(
                1, -1, current_new_xyz_grid_centers.shape[0],
                current_new_xyz_grid_centers.shape[1])
            interpolated_feats_o3d = interpolated_feats_o3d.squeeze(
                dim=0).permute((1, 2, 0))
        ...
        if len(interpolated_feats_o3d_list) == 1:
            ...
            interpolated_feats_o3d = torch.cat(interpolated_feats_o3d_list,
                                               dim=0)
        ...
        if self.use_xyz:
            ...
            local_xyz = (new_xyz_grid_centers.view(-1, 1, 3) -
                         near_known_xyz).view(-1, idx_o3d.shape[1], 9)
...
class PVRCNNPlusPlusVoxelSetAbstraction(nn.Module):
    ...
    def __init__(self, voxel_size, point_cloud_range, num_bev_features,
        ...
        src_name = "x_conv3"
        input_channels = 64
        cur_layer, cur_num_c_out = build_local_aggregation_module(
    ...
    def get_voxel_centers(self, voxel_coords, downsample_times, voxel_size,
                          point_cloud_range):
        """
        Args:
            voxel_coords: (N, 3)
        ...
        pc_range = torch.tensor(point_cloud_range[0:3],
                                device=voxel_centers.device).float()
        voxel_centers = (voxel_centers + 0.5) * voxel_size + pc_range
        return voxel_centers

    def sample_points_with_roi(self,
                               rois,
                               points,
        ...
        if points.shape[0] < num_max_points_of_part:
            ...
            while start_idx < points.shape[0]:
                distance = (points[start_idx:start_idx + num_max_points_of_part,
                                   None, :] - rois[None, :, 0:3]).norm(dim=-1)
                min_dis, min_dis_roi_idx = distance.min(dim=-1)
                roi_max_dim = (rois[min_dis_roi_idx, 3:6] / 2).norm(dim=-1)
    ...
    def sector_fps(self, points, num_sampled_points, num_sectors):
        ...
        sector_idx = (point_angles / sector_size).floor().clamp(min=0,
                                                                max=num_sectors)
        xyz_points_list = []
        xyz_batch_cnt = []
        ...
        sampled_points_batch_cnt = torch.tensor(num_sampled_points_list,
                                                device=points.device).int()
        ...
        for i in range(xyz_batch_cnt.shape[0]):
            ...
            current_sum = current_sum + xyz_batch_cnt[i]
            sampled_point_row_splits.append(current_sum_out +
                                            sampled_points_batch_cnt[i])
            current_sum_out = current_sum_out + sampled_points_batch_cnt[i]
        sampled_pt_idxs = o3d_fps(xyz, torch.tensor(row_splits),
                                  torch.tensor(sampled_point_row_splits)).to(
                                      torch.long)
    ...
    def sectorized_proposal_centric_sampling(self, roi_boxes, points):
        ...
        sampled_points = self.sector_fps(points=sampled_points,
                                         num_sampled_points=self.num_keypoints,
    ...
    def interpolate_from_bev_features(self, keypoints, bev_features, batch_size,
        ...
        for k in range(batch_size):
            ...
            point_bev_features = self.bilinear_interpolate_torch(
                cur_bev_features, cur_x_idxs, cur_y_idxs)
            point_bev_features_list.append(point_bev_features)

        point_bev_features = torch.cat(point_bev_features_list,
                                       dim=0)  # (N1 + N2 + ..., C)
    ...
    def aggregate_keypoint_features_from_one_source(
        ...
        if filter_neighbors_with_roi:
            ...
            for bs_idx in range(batch_size):
                ...
                if points[bs_idx][valid_mask].shape[0] == 0:
                    ...
                    point_features_list.append(
                        torch.zeros((10, points[bs_idx].shape[1])).to(
                            points[bs_idx].device))
            ...
            if valid_point_features.shape[0] == 0:
                return torch.zeros(
                    (new_points.shape[0],
                     self.feature_length[self.feature_index])).to(
                         points[bs_idx].device)
    ...
    def forward(self,
                ...
                bboxes,
                spatial_features,
                multiscale_feats=None):  #multiscale_feats
        ...
        point_bev_features = self.interpolate_from_bev_features(
            keypoints, spatial_features, batch_size, bev_stride=8)
        ...
        pooled_features = self.aggregate_keypoint_features_from_one_source(
            ...
            points=points,
            new_points=new_xyz,
            new_points_batch_cnt=new_xyz_batch_cnt,
        ...
        for i in range(len(self.SA_layer_names)):
            cur_coords = multiscale_feats["multi_scale_3d_features"][
                self.SA_layer_names[i] + "_pos"]
            cur_features = multiscale_feats["multi_scale_3d_features"][
                self.SA_layer_names[i]]
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                batch_size=batch_size,
                aggregate_func=self.SA_layers[i],
...
class PVRCNNKeypointWeightComputer(nn.Module):
    ...
    def __init__(self, num_class, input_channels, cls_fc, gt_extra_width,
        ...
        self.cls_fc = cls_fc
        self.cls_layers = self.make_fc_layers(fc_cfg=self.cls_fc,
                                              input_channels=input_channels,
                                              output_channels=num_class)
        self.gt_extra_width = gt_extra_width
        self.cls_loss_func = SigmoidFocalClassificationLoss(alpha=0.25,
                                                            gamma=2.0)
    ...
    def forward(self,
        ...
        targets_dict = self.assign_targets(batch_size, point_coords, gt_boxes,
                                           gt_labels)
        self.targets_dict = targets_dict
    ...
    def get_cls_layer_loss(self, tb_dict=None):
        ...
        cls_loss_src = self.cls_loss_func(point_cls_preds,
                                          one_hot_targets,
                                          weights=cls_weights)
    ...
    def assign_targets(self, batch_size, points, _gt_boxes, _gt_labels):
        ...
        targets_dict = {'point_cls_labels': point_cls_labels}
...
class PVRCNNPlusPlusProposalTargetLayer(nn.Module):
    ...
    def sample_bg_inds(self, hard_bg_inds, easy_bg_inds, bg_rois_per_this_image,
        ...
        if hard_bg_inds.numel() > 0 and easy_bg_inds.numel() > 0:
            hard_bg_rois_num = min(int(bg_rois_per_this_image * hard_bg_ratio),
                                   len(hard_bg_inds))
    ...
    def subsample_rois(self, max_overlaps):
        ...
        easy_bg_inds = ((max_overlaps <
                         self.cls_bg_thresh_lo)).nonzero().view(-1)
        hard_bg_inds = (
            (max_overlaps < self.reg_fg_thresh) &
    ...
    def iou_3d_compute(self, boxes_a, boxes_b):
        ...
        overlaps_bev = self.overlap_2d(boxes_xyxy_a.unsqueeze(dim=0),
                                       boxes_xyxy_b.unsqueeze(dim=0))
    ...
    def get_max_iou_with_same_class(self, rois, roi_labels, gt_boxes,
                                    gt_labels):
        """
        Args:
            rois: (N, 7)
...
class PVRCNNPlusPlusBoxRefinement(nn.Module):
    ...
    def forward(self,
                ...
                roi_labels,
                keypoint_weights,
                roi_target_dict=None,
                _gt_boxes=None,
                _gt_labels=None):
        batch_size = rois.shape[0]

        if roi_target_dict is None:
            targets_dict = self.assign_targets(batch_size, rois, scores,
                                               _gt_boxes, _gt_labels,
                                               roi_labels)
            rois = targets_dict["rois"]
            roi_labels = targets_dict['roi_labels']
        else:
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        rcnn_loss_reg = (rcnn_loss_reg.view(rcnn_batch_size, -1) *
                         fg_mask.unsqueeze(dim=-1).float()).sum() / max(
                             fg_sum, 1)
        ...
        if fg_sum > 0:
            ...
            rcnn_boxes3d = self.box_coder.decode(
                batch_anchors,
                fg_rcnn_reg.view(batch_anchors.shape[0], -1,
                                 code_size)).view(-1, code_size)

            rcnn_boxes3d = self.rotate_points_along_z(
                rcnn_boxes3d.unsqueeze(dim=1), roi_ry).squeeze(dim=1)
    ...
    def generate_predicted_boxes(self, batch_size, rois, cls_preds, box_preds):
        ...
        local_rois[:, :, 0:3] = 0
        batch_box_preds = self.box_coder.decode(local_rois,
                                                batch_box_preds).view(
                                                    -1, code_size)
        batch_box_preds = self.rotate_points_along_z(
            batch_box_preds.unsqueeze(dim=1), roi_ry).squeeze(dim=1)

        batch_box_preds[:, 0:3] += roi_xyz
        batch_box_preds = batch_box_preds.view(batch_size, -1, code_size)
        return batch_cls_preds, batch_box_preds

    def assign_targets(self, batch_size, rois, scores, _gt_boxes, _gt_labels,
                       roi_labels):
        gt_boxes = []
        for i in range(batch_size):
            gt_boxes.append(
                torch.cat([_gt_boxes[i], _gt_labels[i].view(-1, 1)], dim=1))
        with torch.no_grad():
            targets_dict = self.proposal_target_layer.forward(
                rois, scores, gt_boxes, roi_labels)
        ...
        gt_of_rois = self.rotate_points_along_z(
            ...
            angle=-roi_ry.view(-1)).view(batch_size, -1, gt_of_rois.shape[-1])

        # flip orientation if rois have opposite orientation
        heading_label = gt_of_rois[:, :, 6] % (2 * np.pi)  # 0 ~ 2pi
        opposite_flag = (heading_label > np.pi * 0.5) & (heading_label <
                                                         np.pi * 1.5)
        heading_label[opposite_flag] = (heading_label[opposite_flag] +
                                        np.pi) % (2 * np.pi
                                                 )  # (0 ~ pi/2, 3pi/2 ~ 2pi)
        flag = heading_label > np.pi
        heading_label[flag] = heading_label[flag] - np.pi * 2  # (-pi/2, pi/2)
        heading_label = torch.clamp(heading_label,
                                    min=-np.pi / 2,
                                    max=np.pi / 2)
    ...
    def get_global_grid_points_of_roi(self, rois, grid_size):
        ...
        global_roi_grid_points = self.rotate_points_along_z(
            local_roi_grid_points.clone(), rois[:, 6]).squeeze(dim=1)
        global_center = rois[:, 0:3].clone()
        global_roi_grid_points += global_center.unsqueeze(dim=1)
        return global_roi_grid_points, local_roi_grid_points

    def roi_grid_pool(self, batch_size, rois, point_coords, point_features,
                      keypoint_weights):
    ...
    def boxes_to_corners_3d(self, boxes3d):
        ...
        corners3d = self.rotate_points_along_z(corners3d.view(-1, 8, 3),
                                               boxes3d[:, 6]).view(-1, 8, 3)
        corners3d += boxes3d[:, None, 0:3]

        return corners3d.numpy() if is_numpy else corners3d

    def get_corner_loss_lidar(self, pred_bbox3d: torch.Tensor,
...
class WeightedSmoothL1Loss(nn.Module):
    ...
    def forward(self,
        ...
        if self.code_weights is not None:
            diff = diff * self.code_weights.view(1, 1, -1).to(diff.device)
...
class SubmanifoldSparseConv(nn.Module):
    ...
    def forward(self,
        ...
        for feat, in_pos, out_pos in zip(features_list, in_positions_list,
                                         out_positions_list):
            out_feat.append(
                self.net(feat, in_pos.float(), out_pos.float(), voxel_size))
    ...
    def forward(self, features_list, in_positions_list, voxel_size=1.0):
        ...
        for in_positions in in_positions_list:
            out_positions_list.append(
                calculate_grid(in_positions,
                               torch.tensor(self.point_cloud_range),
                               voxel_size,
                               self.padding,
        ...
        for feat, in_pos, out_pos in zip(features_list, in_positions_list,
                                         out_positions_list):
            if self.downsample:
                feat_temp = self.net(feat, in_pos.float(), out_pos.float(),
                                     voxel_size).to(feat.device)
                stride_temp = torch.cat(
                    [torch.zeros_like(feat_temp[0:1, :]), feat_temp])
                stride_voxelised_2 = voxelize(
                    out_pos.float(),
                    torch.LongTensor([0, out_pos.shape[0]]).to(feat.device),
                    torch.tensor([2.0, 2.0, 2.0]).to(feat.device),
                    torch.Tensor([0, 0, 0]).to(feat.device),
                    torch.Tensor([40960, 40960, 40960]).to(feat.device))
                stride_indices_dense = ragged_to_dense(
                    stride_voxelised_2.voxel_point_indices,
                    stride_voxelised_2.voxel_point_row_splits,
                    self.max_num_points, torch.tensor(-1)) + 1
                ...
                normalizer = torch.clamp_min(num_points.view(-1, 1),
                                             min=1.0).type_as(stride_voxels)
                voxel_mean = voxel_mean / normalizer
                out_feat.append(voxel_mean.contiguous())
                stride_positions = torch.tensor(
                    stride_voxelised_2.voxel_coords).to(feat.device).to(
                        torch.float32)
...
class SparseConvBlock(nn.Module):
    ...
    def forward(self, features_list, in_positions_list, voxel_size=1.0):
        out_feat, out_positions_list = self.conv(features_list,
                                                 in_positions_list,
...
class SubmanifoldSparseConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, norm_fn=None):
        ...
        self.conv = SubmanifoldSparseConv(in_channels, out_channels,
                                          kernel_size)
        self.norm_fn = norm_fn(out_channels)
        self.relu = ReLUBlock()

    def forward(self, features_list, in_positions_list, voxel_size=1.0):
        out_feat = self.conv(features_list,
                             in_positions_list,
                             voxel_size=voxel_size)
        out_feat = self.norm_fn(out_feat)
        out_feat = self.relu(out_feat)
