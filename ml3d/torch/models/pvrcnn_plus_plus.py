    ...
    def forward(self, support_xyz, support_features, xyz_batch_cnt, new_xyz, new_xyz_grid_centers, new_xyz_batch_cnt):
        ...
        for i in range(xyz_batch_cnt.shape[0]):
            if i==0:
                ...
                current_new_xyz_grid_centers = new_xyz_grid_centers[0:new_xyz_batch_cnt[i]]
            else:
                current_support_xyz = support_xyz[current_sum:current_sum+xyz_batch_cnt[i]]
                current_support_features = support_features[current_sum:current_sum+xyz_batch_cnt[i]]
                current_new_xyz_grid_centers = new_xyz_grid_centers[current_sum_new:current_sum_new+new_xyz_batch_cnt[i]]
            ...
            interpolated_feats_o3d = o3d_int(current_support_features.view(1,-1,current_support_xyz.view(1, -1, 3).shape[1]), idx_o3d_temp.view(1, -1, 3), weight.view(1, -1, 3))
            interpolated_feats_o3d = interpolated_feats_o3d.view(1,-1,current_new_xyz_grid_centers.shape[0], current_new_xyz_grid_centers.shape[1])
            interpolated_feats_o3d = interpolated_feats_o3d.squeeze(dim=0).permute((1,2,0))
