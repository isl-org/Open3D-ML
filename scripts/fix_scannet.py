...
scannet_frames = Path(
    ...
    "/mnt/beegfs/tier1/vcl-nfs-work/ssheorey/Open3D/Datasets/ScanNet-frames-bev/frames"
)
scannet_frames_out = Path(
    # "/Users/ssheorey/Documents/Open3D/Data/ScanNet/npy-test/frames_npz_df"
    "/mnt/beegfs/tier1/vcl-nfs-work/ssheorey/Open3D/Datasets/ScanNet-frames-bev/frames_easy"
)


def easy_bbox(bbox_file):
    bboxes = np.load(bbox_file)
    assert np.all(bboxes[:, 6]==0), f"Yaw is not zero for {box_file}"
    bboxes_easy = bboxes[
        np.logical_and(bboxes[:, 7] <= 0.25, bboxes[:, 8] >= 16384), :]
    if len(bboxes_easy) > 0:
        np.save(scannet_frames_out / bbox_file.name, bboxes_easy)
        vert_filename = bbox_file.name.replace('_bbox.npy', '_vert.npz')
        os.symlink(scannet_frames / vert_filename,
                   scannet_frames_out / vert_filename)
